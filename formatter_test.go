...
func TestDefaultFormatter(t *testing.T) {
	...
	for level, levelName := range levelNames {
		result := DefaultFormatter(tag, level, "", 0, "some", "message", 123)
...
func TestJsonFormatter(t *testing.T) {
	...
	for level, levelName := range levelNames {
		...
		expect := fmt.Sprintf(`{"timestamp":%d,"level":%d,"levelName":"%s","fileName":"file.go","line":123,"tag":"%s","msg":"some message 123"}`+"\n", ts, level, levelName, tag)
